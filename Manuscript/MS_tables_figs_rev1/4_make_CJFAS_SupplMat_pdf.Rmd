...
*Possibly include alternative figures to Figure 2 if not included in main MS (show F/Fmsy vs SSB/SSBmsy and C/MSY vs F/Fmsy)* (addresses reviewer comment about Kobe plot)
Relationship between long-term average spawning biomass and average F (relative to MSY levels) for each method. Each point represents the median for a given scenario, separated by the source of the retrospective pattern (catch or M).

Relationship between long-term average F and average catch (relative to MSY levels) for each method. Each point represents the median for a given scenario, separated by the source of the retrospective pattern (catch or M).


*The following figures are similar to Figure 5 in the manuscript but show different scenarios.* (addresses reviewer comment about showing additional scenarios for Figure 5, can really go craazy and add F/Fmsy vs SSB/SSBmsy and Catch/MSY vs F/Fmsy for all 16 scenarios if want to overwhelm readers) 
