...
We developed a management strategy evaluation [MSE; e.g., @punt2016mse] to evaluate the suitability of alternative data-limited methods for setting target catches when age-based stock assessments fail. In particular, focus was placed on methods that use survey indices of abundance, or more generally, index based methods (IBMs). The MSE was designed to test common hypothesized sources of retrospective pattern (missing catch or increases in natural mortality), and to evaluate performance of various methods relative to exploitation history and changes in fishery selectivity. Results of this factorial MSE are summarized for quantities of interest that impact fisheries management advice.
