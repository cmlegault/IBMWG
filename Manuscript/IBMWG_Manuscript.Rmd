...
Fourteen methods for setting catches were explored (13 IBMs and the SCAA) and were applied to all 16 scenarios, which created 224 factorial combinations in the study design. For each element of the full factorial combinations, 1,000 simulations were conducted. All simulations began with the same random number seeds to ensure comparability of results among elements of the study design. Two IBMs (AIM and ES-Fstable) had two failed simulations each, which were caused by relatively high catch advice (i.e., requiring relatively high $F$) that triggered errors in the Newton-Raphson iterations used to determine the $F$ that would produce the desired catch. This small number of failures was unlikely to effect results and conclusions, and so were not considered further.
...
<font size="5">*Sensitivity runs*</font>
...
knitr::include_graphics(file.path(getwd(),"MS_tables_figs","PMs_all_scenarios.jpg"))
```
