...
progress <- mse_sim_setup %>% 
  ...
  filter(IBM == "ensemble") %>% 
  mutate(IBM = ifelse(IBM=="ensemble","SCAA","")) %>% 
  I()


scaa_done <- readRDS("settings/scaa_progress.rds")
done <- which(progress$rowid %in% scaa_done$rowid)
progress$user[done] <- scaa_done$user
progress$date_run[done] <- scaa_done$date_run
progress$uploaded[done] <- scaa_done$uploaded
