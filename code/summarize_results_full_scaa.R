...
newfiles <- myfiles[283:346] # can subset using brackets for testing, e.g., myfiles[1:3]
nfiles <- length(newfiles)

# defines names of files that are saved
mainfile_name <- "results/perform-metrics_full_scaa.rds"
dbfile_name <- "results/dbfile_full_scaa.rds"
dbrowid_name <- "results/dbrowid_full_scaa.rds"
...
for (i in 1:nfiles){
  if (!(newfiles[i] %in% dbfile_start$file)){ # skip if already read this file
    ...
    if (dim(thisrds)[1] > 0){ # skip if nothing in the file
      ...
      if(!is.null(mse_output$IBM[[1]]) & mse_output$IBM[[1]] == "SCAA"){ # make sure IBM=SCAA runs
