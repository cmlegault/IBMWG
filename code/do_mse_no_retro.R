...
#  future::plan(future::multisession)
  ...
  system("git pull")
  progress <- readRDS(file = "settings/no_retro_progress.rds")
  ...
  saveRDS(progress, file = "settings/no_retro_progress.rds")
  #commit back to the repo so other users don't duplicate your efforts
  system('git commit -am "updates no retro progress table"')
  system("git push")
  # #################################
  xx <- mse_sim_setup %>% 
    unnest(specs) %>% 
    filter(retro_type == "Catch",
           n_selblocks == 1,
           catch.mult == 1) %>% 
    I()
  ...
  mse_sim_todo <- mse_sim_setup %>% 
    ...
    mutate(IBM = map_chr(specs, "IBM"),
           specs = map(specs, ~change_input(input = .x, change = list(retro_type="None")))) %>% 
...
#   saveRDS(mse_check, file = "mse_check.rds")
  ...
  saveRDS(progress, file = "settings/no_retro_progress.rds")
  #commit back to the repo so other users don't duplicate your efforts
  system('git commit -am "adds completed runs to no retro progress table"')
  system("git push")
  #################################
