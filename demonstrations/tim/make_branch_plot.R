...
#  write_to_google = FALSE
  ...
  mse_sim_todo <- mse_sim_setup %>% filter(seed == 7084879) %>%
    ...
    filter(catch_mult == 1 & Fhist == 2 & n_selblocks == 1 & retro_type == "Catch") %>%
...
#    select(-input) %>% 
 ...
 # ) #ends profvis
  ...
  ssb = sapply(mse_output$wham, function(x) x$result$true_sim$SSB)
  png("demonstrations/tim/ssb_branch.png", family = "Times", units = "in", type = "cairo-png", height = 10, width = 10, res = 500)
  par(oma = c(0,0,0,0), mar = c(5,5,1,1))
  matplot(1970:2059, ssb/1000, type = 'n', lty = 1, ylab = "SSB (kmt)", xlab = "Year", cex.lab = 2, cex.axis = 1.5)
  grid(col = gray(0.7), lwd = 2)
  box(lwd = 2)
  matplot(1970:2059, ssb/1000, type = 'l', lty = 1, add = TRUE)
  dev.off()

  F = sapply(mse_output$wham, function(x) x$result$true_sim$FAA_tot[,10])
  png("demonstrations/tim/F_branch.png", family = "Times", units = "in", type = "cairo-png", height = 10, width = 10, res = 500)
  par(oma = c(0,0,0,0), mar = c(5,5,1,1))
  matplot(1970:2059, F, type = 'n', lty = 1, ylab = "F", xlab = "Year", cex.lab = 2, cex.axis = 1.5)
  grid(col = gray(0.7), lwd = 2)
  matplot(1970:2059, F, type = 'l', lty = 1, add = TRUE)
  dev.off()

  R = sapply(mse_output$wham, function(x) x$result$true_sim$NAA[,1])
  png("demonstrations/tim/recruit_branch.png", family = "Times", units = "in", type = "cairo-png", height = 10, width = 10, res = 500)
  par(oma = c(0,0,0,0), mar = c(5,5,1,1))
  matplot(1970:2059, R/1000, type = 'n', lty = 1, ylab = "Recruitment (10^6)", xlab = "Year", cex.lab = 2, cex.axis = 1.5)
  grid(col = gray(0.7), lwd = 2)
