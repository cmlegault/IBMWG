...
#reduce catch in appropriate period with ramp if desired
y2 = change_catch_sim(sim = y, catch_ratio = 1/cratio, year_change = 2010, years = 1970:2019)

#see the differences in the aggregated catch
cbind(y$agg_catch, y2$agg_catch)
...
#set.seed(sim.seeds[669,15]) 
y = om_wham$simulate(complete= TRUE)
y = change_catch_sim(sim = y, catch_ratio = 1/cratio, year_change = 2010, years = 1970:2019)
...
for(i in 1:40)
{
  om_wham$env$data$proj_Fcatch[i] = cratio * catch_advice #bump up catch that is actually taken out of the stock
  om_wham$env$data$proj_F_opt[i] = 5 #specify catch
  set.seed(123)
  #reduce observed catch in projection years
  sim_data_series[[i]] = change_catch_sim(sim = om_wham$simulate(complete = TRUE), catch_ratio = 1/cratio, year_change = 2010, years = 1970:2019)
  sim_data_series[[i]]$agg_catch_proj[i, ] = sim_data_series[[i]]$agg_catch_proj[i, ]/cratio #have to change catch in projection years
