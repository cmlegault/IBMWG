...
plot(temp$years, y$SSB[1:temp$input$data$n_years_model], ylab = "SSB", xlim = range(temp$years_full), xlab = "Year", type = "b", ylim =c(0,1500000))
input_i = temp$input
...
for(i in 1:40)
{
  temp$env$data$proj_Fcatch[i] = catch_advice
  temp$env$data$proj_F_opt[i] = 4
  ...
  catch_advice = ifelse(sim_data_series[[i]]$SSB[temp$input$data$n_years_model+i]>SSBlim, Flim, 0.9* sim_data_series[[i]]$FAA_tot[temp$input$data$n_years_model+i,na])* Flim
}

#this does the loop the same as simple_example_2.R just to show that the time series is the same.
temp = fit_wham(x, do.fit = FALSE)
sim_data_series = list()
lines(temp$years, y$SSB[1:temp$input$data$n_years_model], type = "b", col = "red")
input_i = temp$input
SSBlim = exp(rep$log_SSB_MSY[1]) #165,000 mt
Flim = exp(rep$log_FMSY[1]) #165,000 mt
catch_advice = ifelse(y$SSB[temp$input$data$n_years_model]>SSBlim, Flim, 0.9*sum(rep$F[y$n_years_model,]))
for(i in 1:40)
{
  ...
  set.seed(sim.seeds[669,15])
  sim_data_series[[i]] = temp$simulate(complete = TRUE)
  lines(temp$years_full[1:(temp$input$data$n_years_model+i)], sim_data_series[[i]]$SSB[1:(temp$input$data$n_years_model+i)], col = "red")
  #do assessment method (AIM, ASAP, etc.)
    #Example for fitting WHAM assessment model in feedback period
    #tdat = input_i
    #tdat$data = sim_data_series[[i]]
